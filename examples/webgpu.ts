// from https://github.com/crowlKats/webgpu-examples
...
import {
  ...
  copyToBuffer,
  createCapture
} from "https://raw.githubusercontent.com/crowlKats/webgpu-examples/main/utils.ts";
import { gmath } from "https://raw.githubusercontent.com/crowlKats/webgpu-examples/main/deps.ts";
...
class Framework {
  ...
  async loop() {
    ...
    // TODO: this is broken. need to figure out how to copy buffer into window
    setInterval(() => {
      if (win.open) {
        this.render(encoder, texture.createView());
        copyToBuffer(encoder, texture, outputBuffer, this.dimensions);
        win.updateWithBuffer(Uint8Array.from(outputBuffer));
      } else {
